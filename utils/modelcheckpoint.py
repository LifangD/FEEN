	...
	def bert_epoch_step(self, state, current):
		...
		if self.save_best_only:
			if self.monitor_op (current, self.best):
				...
				torch.save (model_to_save.state_dict (), os.path.join (self.base_path, "pytorch_model.bin"))
				logger.info ("new_best_model_saved!")

				# output_config_file = self.base_path / 'configs.json' # model.config
				# with open(str(output_config_file), 'w') as f:
				#     f.write(model_to_save.config.to_json_string()) # already saved in model_to_save.save_pretrained
				state.pop ("model")
