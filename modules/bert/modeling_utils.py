...
class PreTrainedModel (nn.Module):
	r""" Base class for all models.
		...
		Class attributes (overridden by derived classes):
			...
			- ``base_model_prefix``: a string indicating the attribute associated to the base model in derived classes of the same architecture adding src on top of the base model.
	"""
	config_class = None
	...
	def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
		...
		The model is set in evaluation mode by default using ``model.eval()`` (Dropout src are deactivated)
		...
		if pretrained_model_name_or_path in cls.pretrained_model_archive_map:
			...
			if from_tf:
				...
				archive_file = os.path.join (pretrained_model_name_or_path, WEIGHTS_NAME)  # pytorch_model.bin
		else:
			if from_tf:
				# Directly load from a TensorFlow checkpoint
		...
		# Set model in evaluation mode to desactivate DropOut src by default
		model.eval ()
