...
class CRFLoss_vb(nn.Module):
    ...
    def trans_target(self,target,tag_size):
        '''

        :param tagert:  seq_len*batch_size
        :return:
        '''
        new_target = target.clone()
        for i in range(1,target.size(0)):
            new_target[i] = target[i-1]+ tag_size*target[i]
        return new_target
    ...
    def forward(self, scores, target, mask):
        ...
        # calculate sentence score， the gold score
        new_target = self.trans_target(target,self.tagset_size)
        tg_energy = torch.gather(scores.view(seq_len, bat_size, -1), 2, new_target).view(seq_len, bat_size)  # seq_len * bat_size
...
class CRFLoss_vb_PA(nn.Module):
    ...
    def forward(self, scores, target, mask):
        ...
        args:
            scores (seq_len, bat_size, target_size_from, target_size_to) : crf scores
            target (seq_len, bat_size, target_size) : golden state  # different!
            mask (size seq_len, bat_size) : mask for padding
        return:
            loss
        """
        ...
        for idx, cur_values in seq_iter:
            ...
            # target: 需要mask的地方，target[idx]=1, 转换为-inf 在概率计算时贡献为0; 排除的是其他tag的可能性，就像固定了start 和 end 一样！
